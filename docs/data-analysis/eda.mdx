...
[
    ...
    {
        "item_type": "radio",
        "question": "DataFrame의 범주형 변수(예: `df['model']`)에서 각 고유값(예: 'Avante', 'Sonata')이 몇 번씩 나타나는지 그 빈도를 계산하는 데 사용되는 메소드는 무엇입니까?",
        "options": [
            "df.model.unique()",
            "df.model.nunique()",
            "df.model.count()",
        ...
        "hint": "'값(value)'과 '개수(counts)'를 의미하는 두 영어 단어가 합쳐진 이름의 메소드입니다.",
        "solution": "df.model.value_counts()"
    }
]
} />
