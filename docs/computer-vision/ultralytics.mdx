...
<QuizComponent quizId="ultralytics" quizItems={
    [
    {
        "item_type": "checkbox",
        "question": "YOLO 모델들에 대한 설명으로 올바른 것을 모두 고르세요",
        "options": [
            "ultralytics 라이브러리를 사용하면 COCO 데이터에 사전학습된 물체 탐지 모델을 사용할 수 있다",
            "yolo 중에서는 n 모델이 가장 빠르다",
            "yolo 중에서는 x 모델이 가장 크다",
            "yolo 중에서는 m 모델의 성능이 가장 높다"
        ],
        "hint": "YOLO 모델은 크기에 따라 속도와 성능의 트레이드오프 관계를 가집니다. 일반적으로 모델 이름 뒤에 붙는 n, s, m, l, x는 크기를 나타냅니다.",
        "solution": [
            "ultralytics 라이브러리를 사용하면 COCO 데이터에 사전학습된 물체 탐지 모델을 사용할 수 있다",
            "yolo 중에서는 n 모델이 가장 빠르다",
            "yolo 중에서는 x 모델이 가장 크다"
    ...
    {
        "item_type": "radio",
        "question": "model(img, conf=0.8)이라고 하면 어떤 결과가 나오게 되나요?",
        "options": [
            "80% 이상 확신하는 물체만 탐지",
            "전체 물체 중에 80%의 물체만 탐지",
            "크기가 80% 이상인 물체만 탐지"
        ],
        "hint": "conf 파라미터는 'confidence threshold'의 약자로, 모델이 예측한 결과에 대한 신뢰도를 의미합니다.",
        "solution": "80% 이상 확신하는 물체만 탐지"
    }
]
} />
