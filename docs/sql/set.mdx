...
<QuizComponent quizId="set" quizItems={
  ...
  {
    "item_type": "radio",
    "question": "첫 번째 SELECT문의 결과에서 두 번째 SELECT문의 결과를 제외한 차집합을 구하고자 할 때 Oracle DBMS에서 사용하는 명령어는 무엇인가?",
    "options": [
      "EXCEPT",
      "DIFFERENCE",
      "MINUS",
    ...
    "hint": "Oracle 데이터베이스에서 사용하는 차집합 연산자입니다. (참고: SQL Server나 PostgreSQL에서는 EXCEPT를 사용합니다)",
    "solution": "MINUS"
  },
  {
    "item_type": "long",
    ...
    "solution": "SELECT BIZ_REG_NO FROM TB_FINANCIAL_STATEMENT WHERE SUBSTR(FS_YM, 1, 4) = '2023' AND FS_TYPE = 'ANNUAL' INTERSECT SELECT BIZ_REG_NO FROM TB_CREDIT_INFO WHERE CREDIT_GRADE >= 'A';"
  },
  {
    "item_type": "number",
