...
*   데이터 준비 (종양 데이터 특성 X 사용).
    ...
    y_labels = cc.iloc[:, 0].map({'M': 1, 'B': 0}) # 라벨은 시각화에만 사용
    ```
*   **PCA 수행:** (n_components=2 : 2차원으로 축소)
    ...
    # PCA는 분산 기반이므로, 변수 스케일링 필수
    scaler = StandardScaler()
    X_scaled = scaler.fit_transform(X)

    pca = PCA(n_components=2)
    X_pca = pca.fit_transform(X_scaled) # 학습 및 변환 동시 수행
    ```
...
*   PCA로 변환된 2차원 데이터(X_pca)를 산점도로 시각화. 점 색깔은 원래 라벨(y_labels)로 구분.
    ...
    scatter = plt.scatter(X_pca[:, 0], X_pca[:, 1], c=y_labels, cmap='viridis', edgecolor='k', alpha=0.7)
    plt.xlabel('Principal Component 1')
    plt.ylabel('Principal Component 2')
    plt.title('PCA of Cancer Dataset (2 Components)')
    plt.legend(handles=scatter.legend_elements()[0], labels=['Benign (B)', 'Malignant (M)']) # 범례 추가
    plt.grid(True)
    plt.show()
    ```
