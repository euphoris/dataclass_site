...
*   데이터 로드.
    ```python
    import pandas as pd
    of = pd.read_csv('old_faithful.tsv', sep='\t') # 탭으로 구분된 파일
    of.head()
    ```
*   시각화 (대기 시간 vs 분출 시간).
    ```python
    import matplotlib.pyplot as plt
    plt.scatter(of['waiting'], of['eruptions'])
...
*   **표준화:** K-Means는 거리 기반 알고리즘이므로, 변수 간 단위(scale) 다르면 표준화 필요.
    ...
    # 클러스터링할 변수만 선택하여 표준화
    X_scaled = scaler.fit_transform(of[['waiting', 'eruptions']])
    ```
...
*   군집 라벨 할당:
    ...
    of['cluster'] = cluster_labels
    ```
*   결과 시각화:
    ```python
    plt.scatter(of['waiting'], of['eruptions'], c=of['cluster'], cmap='viridis') # c=군집라벨, cmap=색상맵
    # 군집 중심점 표시 (선택 사항)
