...
*   특정 단어(예: '모발')의 인덱스(번호) 찾기
    ...
    try:
        ...
        print("'모발' not in vocabulary")
        i = -1 # or handle error appropriately
    ```
...
*   특정 단어(i번째, 예: '모발')와 유사도가 높은 단어 10개 찾기
    ...
    if i != -1:
        # Get similarity scores for the i-th word
        sim_i = sim[i]
        # Get indices that would sort sim_i in ascending order
        # We use argsort to get indices instead of sorted values
        s = np.argsort(sim_i)

        # Get top 10 most similar words (indices)
        # s[-1] is the index of the word itself (similarity=1)
        # s[-2] is the most similar word, s[-3] the second, etc.
