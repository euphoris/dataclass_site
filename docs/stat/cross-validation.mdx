...
threshold = 0.01
changed = True
best_model = ols(depvar + ' ~ 1', data=train_df).fit()
best_mse = np.inf # 최고의 MSE를 ∞으로 설정 (줄여나갈 것이므로)

while changed:
    excluded = list(set(features) - set(included))
    best_feature = None
    for new_column in excluded:
        model = ols(depvar + ' ~ ' + ' + '.join(included + [new_column]), data=train_df).fit()
    ...
    if changed:
        ...
        print('Add', best_feature, 'MSE:', best_mse)
```
...
threshold = 0.01
changed = True
best_model = ols(depvar + ' ~ ' + ' + '.join(included), data=train_df).fit()
best_mse = np.inf
...
while changed:
    ...
    for remove_feature in included:
        model = ols(depvar + ' ~ ' + ' + '.join(list(set(included) - set([remove_feature]))), data=train_df).fit()
        mse = mean_squared_error(test_df[depvar], model.predict(test_df))
        if mse < best_mse - threshold:
    ...
    if changed:
        ...
        print('Remove', best_feature, 'MSE:', best_mse)
```
